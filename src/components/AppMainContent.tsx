  ...
  return (
    <article className={styles.meetingCard}>
      <div className={styles.meetingIcon} aria-hidden="true">
        <Icon icon={VideoMeeting} size={20} />
      </div>
...
export function AppMainContent({
  ...
  const isWorkspace = activeRailItem === "workspace";
  ...
  switch (activeRailItem) {
    ...
    case "agents":
      railContent =
        agentsView === "manage" ? <ManageAgentsPage /> : <AgentsPage />;
      break;
    case "vibe":
      railContent = <VibePage />;
      break;
    case "workflows":
      railContent = <WorkflowsPage />;
      break;
    case "notetaker":
      railContent = <NotetakerPage />;
      break;
    case "favorites":
    case "more":
      railContent = <UtilityPage activeRailItem={activeRailItem} />;
      break;
    default:
      railContent = null;
  }
  ...
  return (
    <Box className={styles.mainContentRoot}>
      <Box
        className={styles.workspaceLayer}
        aria-hidden={!isWorkspace}
        style={isWorkspace ? undefined : { display: "none" }}
      >
        <WorkspacePage>{renderWorkspace()}</WorkspacePage>
      </Box>
      {!isWorkspace && railContent && (
        <Box className={styles.railLayer}>{railContent}</Box>
      )}
    </Box>
  );
}
